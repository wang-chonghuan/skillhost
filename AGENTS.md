...
Unless bullets are genuinely necessary or the user explicitly asks for them, do not use bullet points in user-facing replies. Prefer clear, connected prose in moderately sized paragraphs.
...
**These guidelines are working if:** fewer unnecessary changes in diffs, fewer rewrites due to overcomplication, clarifying questions come before implementation rather than after mistakes, and important flows rely on one explicit source of truth instead of hidden fallback behavior.

## Project Rules

### Git workflow rule

Do not push unless the user explicitly asks. After a successful rebase, if the user explicitly asks to sync the rebased branch, use `git push --force-with-lease` as the canonical push command instead of a normal push.
